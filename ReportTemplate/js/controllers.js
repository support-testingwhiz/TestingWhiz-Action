    ...
    $scope.expFile = function () {
      angular.forEach($scope.steps, function (value) {
        ...
        if (value.log) {
          ...
          $scope.seqNumber = value.sequencenumber
            ? value.sequencenumber.replace('.', ' : ')
            : ' : '
          $scope.logValue = value.log
            ? value.log.replace(/(\r\n|\n|\r)/gm, '')
            : ''
          if (
            value.action.name == 'Pink' ||
            value.action.name == 'Blue' ||
            value.action.name == 'Green' ||
            value.action.name == 'Purple' ||
            value.action.name == 'Black'
          ) {
            $scope.lastIndex = value.log.lastIndexOf(' ')
            $scope.logValue = value.log.slice(0, $scope.lastIndex + 1)
          }
          $scope.logs.push(
            'Step#' + $scope.seqNumber + $scope.logValue + '\r\n'
          )
        } else if (value.multipleLogsForCondtion.length > 0) {
          $scope.index = 1
          angular.forEach(value.multipleLogsForCondtion, function (log) {
            $scope.seqNumber = value.sequencenumber
              ? value.sequencenumber.replace('.', '.' + $scope.index + ' : ')
              : ' : '
